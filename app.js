...
const DEFAULTS = {
  ...
  customSavings:null, customDebt:null, bills:[], debts:[], debtStrategy:'balanced', paycheckHistory:[], approvedPlan:null, reserveMemory:{},
...
function rollbackReserveContributions(snapshot){
  if(!snapshot?.reserveContributions?.length)return;
...
function paycheckPlan(){
  ...
  const managedDebtTotal=totalDebtBalance(),targetDebt=debtTarget();const desiredDebt=Math.min(clamp(data.debtGoal,0,1e9),managedDebtTotal);
  ...
  const rememberedReserve=[...dueNowBills,...upcomingBills].reduce((s,b)=>s+Number(b.alreadyProtected||0),0);return {paycheck,balance,available,today,nextPay,reserveEnd,planningEnd,dueNowBills,upcomingBills,laterBills,dueNow,upcomingTotal,reserveTarget,rememberedReserve,desiredSavings,desiredDebt,targetDebt,managedDebtTotal,payNow,reserve,savings:chosenSavings,debtPayment:chosenDebt,safeToSpend,shortfall:Math.max(0,dueNow-payNow),reserveShortfall:Math.max(0,reserveTarget-reserve)};
}
...
function recommendation(p){
  ...
  if(p.upcomingBills.length)return`Your immediate bills are covered. I recommend protecting ${money(p.reserve)} from this check for future bills before treating the rest as available. ${why} Then move ${money(p.savings)} to savings${p.debtPayment?`, send ${money(p.debtPayment)} to ${p.targetDebt?.name||'debt'}`:''}. Your TRUE safe-to-spend amount is ${money(p.safeToSpend)}.`;
  return`Pay ${money(p.payNow)} now. No future-bill reserve is needed inside your current look-ahead window, so move ${money(p.savings)} to savings${p.debtPayment?`, send ${money(p.debtPayment)} to ${p.targetDebt?.name||'debt'}`:''}, and keep flexible spending at or below ${money(p.safeToSpend)} until payday.`
}
...
$('approvePlan').onclick=()=>{
  ...
  save();$('approvalStatus').textContent=`Payday plan approved. Dexx remembered ${money(reserveContributions.reduce((s,x)=>s+x.amount,0))} in bill reserves for your next paycheck.`;
};
...
$('chatForm').addEventListener('submit',e=>{e.preventDefault();const text=$('chatInput').value.trim();if(!text)return;const c=calc();$('dexxReply').textContent=`Based on this payday, cover ${money(c.payNow)} now, protect ${money(c.reserve)} for upcoming bills, save ${money(c.savings)}, pay ${money(c.debtPayment)} toward ${c.targetDebt?.name||'debt'}, and limit flexible spending to ${money(c.safeToSpend)}.`;$('chatInput').value=''});
